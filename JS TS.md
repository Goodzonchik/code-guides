...
#### 90. Используйте `alias` `todoAny` для типа `any`.

Если есть необходимость указать тип `any`, например, на момент разработки до конца не известна модель данных и т.п., можно использовать тип `todoAny`.

**Почему?** `todoAny` - своим названием показывает, что это технический долг. А также его легче найти в коде, чем `any`, так при поиске найдутся слова "compANY" и подобные.
