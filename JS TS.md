...
#### 2. При поверхностном копировании объектов и массивов лучше использовать оператор «…», а не `Object.assing`.
...
#### 7. Сокращения или буквенные аббревиатуры всегда должны быть в верхнем или нижнем регистре.
...
#### 14. Для булевых типов применять сокращение.
...
#### 15. Для строк и чисел применяйте явное сравнение.
...
#### 25. Импорты на нескольких строках должны быть с отступами как у многострочных литералов массива и объекта.
...
#### 38. Если ваш управляющий оператор (`if`, `while` и т.д.) слишком длинный или превышает максимальную длину строки, то каждое условие можно поместить на новую строку. Логический оператор должен располагаться в начале строки.
...
#### 66. Функции с многострочным определением или запуском должны содержать такие же отступы, как и другие многострочные списки в этом руководстве: с каждым элементом на отдельной строке, с запятой в конце элемента.
...
#### 67. Используйте фигурные скобки для `case` и `default`, если они содержат лексические декларации (например, let, const, function, и class).
...
#### 68. Тернарные операторы не должны быть вложены и в большинстве случаев должны быть расположены на одной строке.
...
#### 69. Избегайте ненужных тернарных операторов.
...
#### 70. Используйте точечную нотацию для доступа к свойствам.
...
#### 71. Используйте `extends` для наследования.

**Почему?** Это встроенный способ наследовать функциональность прототипа, не нарушая `instanceof`.

#### 72. Методы могут возвращать this, чтобы делать цепочки вызовов.
...
#### 73. У классов есть конструктор по умолчанию, если он не задан явно. Можно опустить пустой конструктор или конструктор, который только делегирует выполнение родительскому классу.

**eslint: no-useless-constructor**

#### 74. Избегайте дублирующих членов класса.
...
#### 75. Используйте операторы return внутри функций обратного вызова в методах массива. Можно опустить return, когда тело функции состоит из одной инструкции, возвращающей выражение без побочных эффектов.
...
#### 76. Если массив располагается на нескольких строках, то используйте разрывы строк после открытия и перед закрытием скобок.
...
#### 77. При обращении к нескольким свойствам объекта используйте деструктуризацию объекта.
...
#### 78. Используйте деструктуризацию массивов.
...
#### 79. Когда вам необходимо использовать анонимную функцию, используйте стрелочную функцию.
...
#### 80. Если тело функции состоит из одного оператора, возвращающего выражение без побочных эффектов, то опустите фигурные скобки и используйте неявное возвращение. В противном случае, сохраните фигурные скобки и используйте оператор return.
...
#### 81. Всегда оборачивайте аргументы круглыми скобками для ясности и согласованности.
...
#### 82. Не используйте итераторы. Применяйте функции высшего порядка вместо таких циклов как `for-in` или `for-of`.
...
#### 83. Использовать для сортировки `Intl-API`.

#### 84. У методов/функций тип входных и выходных указывать все возможные значения через `|`.
...
#### 85. Именовать поля в `enum` с большой буквы.

**Почему?** На бэкенде принято называть поля в `enum` с большой буквы. Для поддержания консистентности.

#### 86. Для очистки значения поля использовать `null`, а не `undefined`.
...
#### 87. Обязательно приводить значение логических вычислений к `boolean`, если есть вероятность возврата другого значения.
...
#### 88. Выносить тело запроса в константу/переменную.
...
#### 89. Используйте отступ в 4 пробела.